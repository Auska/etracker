...
- **Release mode**: `cmake -DCMAKE_BUILD_TYPE=Release .` (includes optimization flags)
...
- `--no-tcp` - Disable TCP
- `--no-udp` - Disable UDP
- `--no-locations` - Disable GeoIP database
...
Displays real-time server statistics including:
- Load average
- Requests per second
- Memory usage
- Active connections
...
**Setup**: Initialize web resources:
```bash
./scripts/initWeb
```
...
- [`scripts/initWeb`](scripts/initWeb) – Initialize web resources (download jQuery)

### Documentation

- [`CMakeLists.txt`](CMakeLists.txt) – CMake build configuration
- [`HOW_TO_ADD_LOCALE.md`](HOW_TO_ADD_LOCALE.md) – Guide for adding locale support
